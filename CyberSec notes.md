...
-	KeePassX, Clipperz, Password Gorilla, Gpassword Manager, and Password Safe are some of the most widely used password managers.
...
----
   ...
   **Users and networks are protected by UTM:**
...
# Physical Security:
  ...
  Physical security refers to safeguards in place to prevent illegal access to facilities, equipment, and resources, as well as to protect workers and property from harm. CCTV monitoring, access control, perimeter intrusion detection, deterrent systems, and other systems designed to safeguard people and property are examples of physical security systems.
  

  **1.Detection:** You have no means of knowing when an unauthorised user requested a key or exceeded the time limit if you employ the manual key control system. The following sorts of detection methods exist:

  - Alarm systems and sensors can be deployed to inform security staff if unauthorised access is attempted. Sensors such as perimeter sensors, motion sensors, and other types of sensors are included.
  - Video Surveillance: If an attack has already occurred and a camera is put at the location of attack, surveillance cameras can be used to detect the attack. It is possible to utilise video footage that has been recorded.


  **2.Controlling Access:** These techniques are used to keep track of and control traffic at certain entry points. The following methods are included in Access Control:

  -	Gates, doors, locks, and other mechanical access control systems are examples.

  -	Electronic Access Control: These are used to monitor and control huge groups of people, with user life cycles, dates, and individual access points all being taken into account.

  


  **3.Personnel in charge of security:** They play a crucial role in all security layers. They are in charge of a wide range of responsibilities, such as:

  - Managing electronic access control systems.
  -	Taking action in the event of an alarm.
  -	Monitoring and analysing video footage, as well as a variety of other tasks
